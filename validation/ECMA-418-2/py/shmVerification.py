...
from refmap_psychoacoustics.metrics.ecma418_2 import (shmTonalityECMA,
                                                      shmLoudnessECMAFromComp,
                                                      shmRoughnessECMA)


# %% shmVerification
def shmVerification(saveData=False, outPlot=False):
    ...
    p, sampleRateIn = sf.read(os.path.join(inputPath,
                                           'BusyStreet1_0530-0600.wav'))

    # %% Calculate metric values

    tonalitySHM = shmTonalityECMA(p, sampleRateIn, axisN=0,
                                  soundField='freeFrontal', waitBar=True,
                                  outPlot=outPlot)

    loudnessSHM = shmLoudnessECMAFromComp(tonalitySHM['specTonalLoudness'],
                                          tonalitySHM['specNoiseLoudness'],
                                          outPlot=outPlot, binaural=True)

    roughnessSHM = shmRoughnessECMA(p, sampleRateIn, axisN=0,
                                    soundField='freeFrontal', waitBar=True,
                                    outPlot=outPlot, binaural=True)

    if saveData:
        # %% store outputs as .mat files
        savemat(os.path.join(outputPath,
                             "BusyStreet1_0530-0600_tonalitySHM_Py.mat"),
                tonalitySHM, appendmat=False, do_compression=True,
                oned_as='column')

        savemat(os.path.join(outputPath,
                             "BusyStreet1_0530-0600_loudnessSHM_Py.mat"),
                loudnessSHM, appendmat=False, do_compression=True,
                oned_as='column')

        savemat(os.path.join(outputPath,
                             "BusyStreet1_0530-0600_roughnessSHM_Py.mat"),
                roughnessSHM, appendmat=False, do_compression=True,
                oned_as='column')

    return tonalitySHM, loudnessSHM, roughnessSHM
