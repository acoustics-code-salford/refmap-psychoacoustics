...
filename <- "PtsABdAnnoyMnEnvLAeqModSxTnPkA"

if (saveplots){
  ggsave(filename=paste(filename, ".svg", sep=""), width=11, height=4,
         path=file.path(outFigPath, "svg"), system_fonts = list('serif'="Times New Roman"))
  unlink(paste(filename, ".svg", sep=""))
...
ggplot(data=predictProbs) +
  ...
  geom_point(mapping=aes(x=response, y=fit), size=2.5, shape=1, colour=mycolours[6], alpha=0.5) +
...
filename <- "PtsABdHighAnnoyBasicPred"
