...
SNRCats <- c("Baseline", "-16", "-10", "-4", "2", "8")
UASLAeqCats <- c("Baseline", "42", "48", "54", "60")
...
# make numeric dataframe with selected variables for random forest model, and convert categorical variables to ordinal numeric variables
# assign rownames from stimData dataframe to new dataframe
stimDataNum <- data.frame(UASEvents=stimData[, "UASEvents"])

stimDataNum <- cbind(stimDataNum,
                     ...
                     stimData[, which(colnames(stimData)=="UASTonalECMAAvgMaxLR"):
                                which(colnames(stimData)=="UASSharpvBISO105ExMaxLR")],
...
if (loadExisting){
  # annoy mean results
  resdAnnoyMnFitAB <- utils::read.csv(file.path(outDataPath, "\\PtsABCRFdAnnoyMnOOBFit.csv"), sep = ",", row.names=1)
  
  resdAnnoyMnPermImpAB <- list()
  wb <- openxlsx2::wb_load(file.path(outDataPath, "\\PtsABCRFdAnnoyMnConPermimp.xlsx"))
  sheetNames<- wb_get_sheet_names(wb)

  for (sheet in sheetNames){
    col_name <- names(openxlsx2::wb_to_df(wb, sheet, row_names=TRUE))[1]
    resdAnnoyMnPermImpAB[[col_name]] <- openxlsx2::wb_to_df(wb, sheet, row_names=TRUE)
  }
  
  # high annoyance results
  resdHiAnnoyFitAB <- utils::read.csv(file.path(outDataPath, "\\PtsABCRFdHiAnnoyOOBFit.csv"), sep = ",", row.names=1)
  
  resdHiAnnoyPermImpAB <- list()
  wb <- openxlsx2::wb_load(file.path(outDataPath, "\\PtsABCRFdHiAnnoyConPermimp.xlsx"))
  sheetNames<- wb_get_sheet_names(wb)

  for (sheet in sheetNames){
    col_name <- names(openxlsx2::wb_to_df(wb, sheet, row_names=TRUE))[1]
    resdHiAnnoyPermImpAB[[col_name]] <- openxlsx2::wb_to_df(wb, sheet, row_names=TRUE)
  }
}
...
if (saveplots){
  ggsave(filename="PtsABdAnnoyMnAbsVarsConPermimp.svg", width=8, height=15, path=file.path(outFigPath, "svg"))
  unlink("PtsABdAnnoyMnAbsVarsConPermimp.svg")
  
  ggsave(filename="PtsABdAnnoyMnAbsVarsConPermimp.pdf", width=8, height=15, path=file.path(outFigPath, "pdf"))
...
allSharpVar <- "dSharpAurISO3PowAvgMaxLR"
...
iVars <- c(allVar, eventVar, ambVar, "dTonalECMAAvgMaxLR", "dTonalSHMInt05ExMaxLR", "dTonalSHMIntAvgMaxLR", "dTonalECMA05ExMaxLR", "dTonalAwSHMAvgMaxLR",	"dTonalAwSHM05ExMaxLR",	"dTonalAwSHMIntAvgMaxLR", 	"dTonalAwSHMInt05ExMaxLR", "dTonLdECMAPowAvgBin", "dTonLdECMA05ExBin", "dTonShpAurSHMPowAvgMaxLR",
           "dTonShpAurSHM05ExMaxLR", "PartTonShpAurSHMPowAvgMaxLR",
           "PartTonShpAurSHM05ExMaxLR", "PartTonShpvBSHMPowAvgMaxLR",
           "PartTonShpvBSHM05ExMaxLR", "PartTonShpWidSHMPowAvgMaxLR",
           "PartTonShpWidSHM05ExMaxLR", "PartTonLdSHMPowAvgBin", "UASTonalECMAAvgMaxLR", "UASTonalSHMInt05ExMaxLR", "UASTonalSHMIntAvgMaxLR", "UASTonalECMA05ExMaxLR", "UASTonalAwSHMAvgMaxLR",	"UASTonalAwSHM05ExMaxLR",	"UASTonalAwSHMIntAvgMaxLR", 	"UASTonalAwSHMInt05ExMaxLR", "UASTonLdECMAPowAvgBin", "UASTonLdECMA05ExBin", "UASTonalAurAvgMaxLR", "UASTonalAur05ExMaxLR", "UASTonalAur10ExMaxLR",
         "UASTonShpAurSHMPowAvgMaxLR", "UASTonShpAurSHM05ExMaxLR")
dVar <- "dAnnoyMean"
...
if (saveplots){
  ...
  ggsave(filename="PtsABdAnnoyMnAllTonalConPermimp.pdf", width=8, height=5.8, path=file.path(outFigPath, "pdf"))
...
if (savedata){
  ...
  openxlsx2::write_xlsx(temp, paste(outDataPath, "\\PtsABCRFdAnnoyMnConPermimp.xlsx",
                                   sep=""),
                        rowNames=TRUE)
}
...
seeds <- c(5641848, 504, 78, 750823, 48659)
...
iVars <- c(absVar, eventVar, ambVar, "UASSharpAurISO3PowAvgMaxLR", "UASSharpAurISO305ExMaxLR", "UASSharpAurSHMPowAvgMaxLR", "UASSharpAurSHM05ExMaxLR", "UASSharpAurISO1PowAvgMaxLR", "UASSharpAurISO105ExMaxLR", "UASSharpvBISO1PowAvgMaxLR", "UASSharpvBISO105ExMaxLR", "UASSharpDINPowAvgMaxLR", "UASSharpDIN05ExMaxLR", "UASSharpAurISO1MedMaxLR",
         "UASTonShpAurSHMPowAvgMaxLR", "UASTonShpAurSHM05ExMaxLR")
dVar <- "dHighAnnoyPc"

seeds <- c(418502, 8496, 514, 23, 365)
...
sharpVar <- "UASSharpAurISO3PowAvgMaxLR"
...
seeds <- c(48796, 84, 51643, 8465, 6985)
...
seeds <- c(41065, 1032, 85641, 5, 3012)
...
seeds <- c(4156, 10865, 6531, 8654, 784)
...
seeds <- c(5312, 645, 4868541, 9, 86541)
...
seeds <- c(86541, 512, 41365, 485106, 614)
...
seeds <- c(54, 14563, 864, 45407, 1086)
...
pBar <- ggplot(resultsOutSQMs1.conimp) + geom_col(aes(x=factor(rownames(resultsOutSQMs1.conimp), levels=rownames(resultsOutSQMs1.conimp)), y=CondPermImp), fill=mycolours[7], width=0.5) + labs(x="Variable", y="Conditional variable permutation importance (% highly annoyed)") + theme(text = element_text(family = "serif"), panel.grid=element_line(color = rgb(235, 235, 235, 100, maxColorValue = 255), linewidth = 0.25, linetype = 2)) + coord_flip(ylim=c(0, 25))
pBar
...
seeds <- c(10, 3654, 64185, 98654, 864)
...
seeds <- c(674853, 8546, 3540, 4, 465)
...
if (saveplots){
  ggsave(filename="PtsABdHiAnnoyAllVarsConPermimp1pc.svg", width=8, height=6, path=file.path(outFigPath, "svg"))
  unlink("PtsABdHiAnnoyAllVarsConPermimp1pc.svg")
  
  ggsave(filename="PtsABdHiAnnoyAllVarsConPermimp1pc.pdf", width=8, height=6, path=file.path(outFigPath, "pdf"))
...
if (savedata){
  ...
  openxlsx2::write_xlsx(temp, paste(outDataPath, "\\PtsABCRFdHiAnnoyConPermimp.xlsx",
                                   sep=""),
                       rowNames=TRUE)
}
...
if (savedata){
  ...
  openxlsx2::write_xlsx(temp, paste(outDataPath, "\\PtsABCRFSummary.xlsx",
                                   sep=""),
                       rowNames=TRUE)
}
...
```{r}
if (savedata){
  save.image(file.path(outDataPath, "PtsABCRFWorkspace.RData"))
}
