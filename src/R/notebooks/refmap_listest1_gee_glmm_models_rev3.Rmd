...
m3Data <- subjDataNoAmb.med[, c(all.vars(m3formula), ID='ID'), drop=FALSE]
...
m3bformula <- update(m3formula, . ~ . - TrialNumberScl)

```

```{r}

# keep StimID alongside the model variables so we can recover it after na.omit
m3bData <- subjDataNoAmb.med[, c(all.vars(m3bformula), "StimID"), drop = FALSE]

mf <- model.frame(m3bformula, data = m3bData)      # na.omit happens here
kept_rows <- as.integer(rownames(mf))               # original row indices actually retained

y_matrix <- model.response(mf)
X_matrix <- model.matrix(m3bformula, data=m3bData)[,-1]
...
nd = cbind(ID=subjDataNoAmb.med$ID, m3bData)
m3bdiag <- glmtoolbox::glmgee(m3bformula, data=nd,
                              family=quasibinomial(link='logit'),
                              id=ID, corstr='exchangeable')

performance::check_collinearity(m3bdiag)
...
# 2. Extract your specific optimal lambda to add a guide line
optimal_lambda <- m3b$lambda.min
...
ggplot(coefficient_paths, aes(x = lambda, y = estimate, color = term)) +
  ...
  # scale_y_log10() +
  geom_vline(xintercept = optimal_lambda, linetype = 'dotted', color = "grey", linewidth = 0.8) +
  ggplot2::annotate("text", x = optimal_lambda, y = max(coefficient_paths$estimate) * 0.55, 
           label = "Optimal Lambda", color = "grey", angle = 90, vjust = -0.5) +
...
m4Data <- subjDataNoAmb.med[, c(all.vars(m4formula), ID='ID'), drop=FALSE]
...
compare_cv_results(list(m1d=m1d_cv, m2=m2_cv, m4=m4_cv))
