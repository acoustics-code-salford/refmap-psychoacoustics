...
m7b <- glmnet::cv.glmnet(x=X_matrix, y=y_matrix, family=quasibinomial(link='logit'),
                         ...
                         standardize=TRUE, lambda.min.ratio = 1e-18, nlambda = 200,
                         parallel=TRUE)
...
ggplot(data=m7predEvt) +
              ...
              scale_y_continuous(breaks=seq(-10, 10, by=1), limits=c(-3, 7)) +
...
m8formula <- dAnnoyanceOrdBetaScl ~ TrialNumberScl +
  Detect0p5dBIntMaxLRScl*I(log10(UASEvents)) +
  I(Detect0p5dBIntMaxLRScl^2)
...
m8_cv <- cv_glmgee(m8, id_col="ID", K=10, seed=124,
                  data=nd, agg_cols="StimID",
                  return_predictions=TRUE,
                  backtransform_fn=\(x) betaTransform(x, -10, 10,
                                                        direction="reverse",
...
modelStep <- glmtoolbox::stepCriterion(model=m8,
                                       criterion='agpc', test='score',
                                       direction='backward',
                                       varest='bias-corrected')
...
```{r}
m8bformula <- update(m8formula, . ~ . - Detect0p5dBIntMaxLRScl:I(log10(UASEvents)))
...
m8b_cv <- cv_glmgee(m8b, id_col="ID", K=10, seed=124,
                  data=nd, agg_cols="StimID",
                  return_predictions=TRUE,
                  backtransform_fn=\(x) betaTransform(x, -10, 10,
                                                        direction="reverse",
...
m8bpredMeans <- marginaleffects::avg_predictions(m8b, newdata=nd,
                                                  by=c("StimID",
                                                       "Detect0p5dBIntMaxLRScl",
                                                       "UASEvents"),
                                                  vcov=vcov(m8b, type='bias-corrected'),
                                                 type='response',
                                                transform=\(x) betaTransform(x, -10, 10,
                                                                             direction='reverse',
                                                                             squeeze='none')) |>
...
R2means <- 1 - sum((stimDataNoAmb$dAnnoyMean - m8bpredMeans$estimate)^2)/sum((stimDataNoAmb$dAnnoyMean - mean(stimDataNoAmb$dAnnoyMean))^2)

eRMSE <- epsilonRMSE(measured_data=stimDataNoAmb, predicted_data=m8bpredMeans,
                     measured_target='dAnnoyMean', measured_ci_low='dAnnoyMeanCI_Low',
                     measured_ci_high='dAnnoyMeanCI_High', fitted='estimate')

# using ggplot, plot the predicted fit against the responses in predictMeans
ggplot(data=cbind(m8bpredMeans,
                  response=stimDataNoAmb$dAnnoyMean,
                  responseCI_Low=stimDataNoAmb$dAnnoyMeanCI_Low,
                  responseCI_High=stimDataNoAmb$dAnnoyMeanCI_High)) +
  theme(panel.grid = element_blank(), aspect.ratio=1, text=element_text(family = "serif", size=16),
...
m9_cv <- cv_glmgee(m9, id_col="ID", K=10, seed=124,
                  data=nd, agg_cols="StimID",
                  return_predictions=TRUE,
                  backtransform_fn=\(x) betaTransform(x, -10, 10,
                                                        direction="reverse",
