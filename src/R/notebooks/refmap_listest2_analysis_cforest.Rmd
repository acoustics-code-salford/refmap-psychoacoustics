...
stimData <- utils::read.csv(stimDatapath, header=TRUE, row.names=1)
...
stimData[['ProbHA20kPc']] <- stimData[['ProbHA20kMean']]*100
stimData[['ProbHA10kPc']] <- stimData[['ProbHA10kMean']]*100
stimData[['dProbHA20kPc']] <- stimData[['dProbHA20kMean']]*100
stimData[['dProbHA10kPc']] <- stimData[['dProbHA10kMean']]*100
...
```{r}
stimDataNum <- data.frame(UASEvents=stimData[, "UASEvents"])

stimDataUAS <- cbind(stimDataNum,
                     ...
                     stimData[, which(colnames(stimData)=="AnnoyanceMean"):
                                which(colnames(stimData)=="dProbHA10kPc")])

row.names(stimDataUAS) <- rownames(stimData)

stimDataAmb <- cbind(stimDataNum,
...
row.names(stimDataAmb) <- rownames(stimData)
...
if (loadExisting){
  ...
  for (sheet in sheetNames){
    col_name <- names(openxlsx2::wb_to_df(wb, sheet, row_names=TRUE))[1]
...
A function to train a conditional-inference random forest (crf) model on input data according to input formula, iterate over input random seeds, average error and variable importance metrics, and output metrics with plotted
...
if (saveplots){
  ggsave(filename=paste(filename, ".svg", sep=""), width=12, height=22, path=file.path(outFigPath, "svg"))
  unlink(paste(filename, ".svg", sep=""))
  
  ggsave(filename=paste(filename, ".pdf", sep=""), width=12, height=22, path=file.path(outFigPath, "pdf"))
  unlink(paste(filename, ".pdf", sep=""))
}
...
if (saveplots){
  ggsave(filename=paste(filename, ".svg", sep=""), width=12, height=8, path=file.path(outFigPath, "svg"))
  unlink(paste(filename, ".svg", sep=""))
...
allVar <- "Detect0p5dBIntEx50MaxLR"
