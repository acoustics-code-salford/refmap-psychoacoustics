...
subjData <- subjData |> dplyr::mutate(IntermitRatioMaxLRScl = IntermitRatioMaxLR/100)
...
stimData <- stimData |> dplyr::mutate(IntermitRatioMaxLRScl = IntermitRatioMaxLR/100)
...
mABdHLAExTypexOpxEnvxLAElgQ.GEE.resD <- residuals(mABdHLAExTypexOpxEnvxLAElgQ.GEE,
                                                     type="deviance", plot.it=TRUE,
                                                     pch=1, col=mycolours[1],identify=10)
mABdHLAExTypexOpxEnvxLAElgQ.GEE.resM <- residuals(mABdHLAExTypexOpxEnvxLAElgQ.GEE,
                                                     type="mahalanobis", plot.it=TRUE,
                                                     col=mycolours[2], identify=10)
mABdHLAExTypexOpxEnvxLAElgQ.GEE.resP <- residuals(mABdHLAExTypexOpxEnvxLAElgQ.GEE,
                                                     type="pearson", plot.it=TRUE,
                                                     pch=1, col=mycolours[3], identify=10)

hist(mABdHLAExTypexOpxEnvxLAElgQ.GEE.resD, breaks=20, main="Deviance residuals")
hist(mABdHLAExTypexOpxEnvxLAElgQ.GEE.resP, breaks=20, main="Pearson residuals")

par(mar = rep(2, 4))

mABdHLAExTypexOpxEnvxLAElgQ.GEE.DfBc <- dfbeta(mABdHLAExTypexOpxEnvxLAElgQ.GEE,
                                                  level='clusters',
                                         coefs=c('UASLAEMaxLRScl'))
mABdHLAExTypexOpxEnvxLAElgQ.GEE.DfBo <- dfbeta(mABdHLAExTypexOpxEnvxLAElgQ.GEE,
                                                  level='observations',
                                         coefs=c('UASLAEMaxLRScl'))

mABdHLAExTypexOpxEnvxLAElgQ.GEE.Cook <- cooks.distance(mABdHLAExTypexOpxEnvxLAElgQ.GEE,
                                                          plot.it=TRUE)

performance::check_collinearity(mABdHLAExTypexOpxEnvxLAElgQ.GEE)
...
### LAE only with events and trial sequence

```{r}

model1 <- mABHLAElgQ.GEE <- glmtoolbox::glmgee(HighAnnoy ~ I(log10(UASEvents)) +
                                                        UASLAEMaxLRScl,
                                                        data=subjDataNoAmb,
                                                        family=binomial(link='logit'),
                                                        id=ID.,
...
geeModelPerformance(mABHLAElgQ.GEE)
...
model1 <- mABHNlgQ.GEE <- glmtoolbox::glmgee(HighAnnoy ~ I(log10(UASEvents)) +
...
geeModelPerformance(mABHLAElgQ.GEE, mABHNlgQ.GEE)
...
model1predictIr <- ggeffects::predict_response(model1, terms=c("IntermitRatioMaxLRScl [all]",
                                                                 "UASEvents [all]"),
                                             margin='marginalmeans', type='fixed', 
                                            vcov=vcov(model1, type="bias-corrected"))
...
p1 <- ggplot(data=model1predictEvt) +
              ...
              geom_point(data=stimDataNoAmb, aes(x=Detect0p1IntMaxLRLogScl, y=dHighAnnoyProp,
                                                 colour=IntermitRatioMaxLRScl, shape=factor(UASEvents)),
                         alpha=0.55, size=2) +
...
p5 <- ggplot(data=model1predictIr) +
              ...
              geom_point(data=stimDataNoAmb, aes(x=IntermitRatioMaxLRScl, y=dHighAnnoyProp,
                                                 colour=factor(UASEvents), shape=factor(UASEvents)),
                         alpha=0.55, size=2) +
              ...
              rescaleTicks(stimDataNoAmb$IntermitRatioMaxLR, stimDataNoAmb$IntermitRatioMaxLRScl, sep=1) +
              scale_y_continuous(breaks=seq(0, 1, by=0.1), limits=c(0, 1), labels=seq(0, 100, by=10)) +
