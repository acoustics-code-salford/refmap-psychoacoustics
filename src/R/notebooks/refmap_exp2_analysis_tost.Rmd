...
# create subsetted pivoted dataframe for left vs right UAS start for annoyance response, sorted by Stim
stim_AnnoyMnDeltas <- stimDataNoAmb[, c('Stimulus', 'UASStart', 'AnnoyanceMean')] |>
  mutate(Stimulus = sub("_left", "", Stimulus)) |>
  mutate(Stimulus = sub("_right", "", Stimulus))|>
  tidyr::pivot_wider(
    names_from = UASStart,
    values_from = AnnoyanceMean
  ) |>
...
tostRes1 <- tsum_TOST(
  m1 = mean(stimDataNoAmb$AnnoyanceMean[stimDataNoAmb$UASStart == "Left"]),
  sd1 = sd(stimDataNoAmb$AnnoyanceMean[stimDataNoAmb$UASStart == "Left"]),
  n1 = sum(stimDataNoAmb$UASStart == "Left"),
  m2 = mean(stimDataNoAmb$AnnoyanceMean[stimDataNoAmb$UASStart == "Right"]),
  sd2 = sd(stimDataNoAmb$AnnoyanceMean[stimDataNoAmb$UASStart == "Right"]),
  n2 = sum(stimDataNoAmb$UASStart == "Right"),
  r12 = cor(stimDataNoAmb$AnnoyanceMean[stimDataNoAmb$UASStart == "Left"], stimDataNoAmb$AnnoyanceMean[stimDataNoAmb$UASStart == "Right"]),
  paired = TRUE,
  var.equal = FALSE,
  eqb = sesoi,
  alpha = 0.05
...
tostRes2 <- tsum_TOST(
  m1 = mean(stim_AnnoyMnDeltas$delta),
  sd1 = sd(stim_AnnoyMnDeltas$delta),
  n1 = length(stim_AnnoyMnDeltas$delta),
  eqb = sesoi,
  alpha = 0.05
)
