...
stimDataBNum <- stimDataBNum[, !(colnames(stimDataBNum) %in% c('UASLAeqMaxLR', 'UASLAEMaxLR', 'UASEventDensity'))]
...
iVars <- names(stimDataBNum)[which(names(stimDataBNum) == 'UASEventPerMin'):which(names(stimDataBNum) == 'UASImpulsSHM05ExMaxLR')]
...
p <- mtryTune(dataIn=stimDataBNum, iVars=iVars, dVar=dVar, seed=seeds[1],
...
stimDataBNum <- rbind(stimDataBNum, stimDataB[stimDataB['Stimulus']
                                              == "A1",
                                              colnames(stimDataBNum)],
                      stimDataB[stimDataB['Stimulus']
                                              == "A2",
                                              colnames(stimDataBNum)])
stimDataBNum <- arrange(stimDataBNum, Stimulus)
...
iVars <- c(absVar, "UASEventPerMin", "UASSharpAurISO3PowAvgMaxLR", "UASSharpAurISO305ExMaxLR", "UASSharpAurSHMPowAvgMaxLR", "UASSharpAurSHM05ExMaxLR", "UASSharpAurISO1PowAvgMaxLR", "UASSharpAurISO105ExMaxLR", "UASSharpvBISO1PowAvgMaxLR", "UASSharpvBISO105ExMaxLR", "UASSharpDINPowAvgMaxLR", "UASSharpDIN05ExMaxLR")
dVar <- "dAnnoyMean"
...
sharpVar <- "UASSharpDINPowAvgMaxLR"
...
iVars <- c(absVar, "UASEventPerMin", "UASTonalECMAAvgMaxLR", "UASTonalSHMInt05ExMaxLR", "UASTonalSHMIntAvgMaxLR", "UASTonalECMA05ExMaxLR", "UASTonLdECMAPowAvgBin", "UASTonLdECMA05ExBin", "UASTonalAurAvgMaxLR", "UASTonalAur05ExMaxLR", "UASTonalAur10ExMaxLR")
dVar <- "dAnnoyMean"
...
iVars <- c(absVar, "UASEventPerMin", "UASTonalECMAAvgMaxLR", "UASTonalSHMInt05ExMaxLR", "UASTonalSHMIntAvgMaxLR", "UASTonalECMA05ExMaxLR", "UASTonalAurAvgMaxLR", "UASTonalAur05ExMaxLR", "UASTonalAur10ExMaxLR")
dVar <- "dAnnoyMean"
...
# Fluctuation strength
iVars <- c(absVar, "UASEventPerMin", "UASFluctSHM10ExBin", "UASFluctSHM05ExBin", "UASFluctFZ10ExMaxLR", "UASFluctFZ05ExMaxLR", "UASFluctOV10ExMaxLR", "UASFluctOV05ExMaxLR")
dVar <- "dAnnoyMean"
...
# Roughness
iVars <- c(absVar, "UASEventPerMin", "UASRoughECMA10ExBin", "UASRoughECMA05ExBin", "UASRoughFZ10ExMaxLR", "UASRoughFZ05ExMaxLR", "UASRoughDW10ExMaxLR", "UASRoughDW05ExMaxLR")
dVar <- "dAnnoyMean"
...
```{r}
# Impulsiveness
iVars <- c(absVar, "UASEventPerMin", "UASImpulsSHMAvgMaxLR", "UASImpulsSHM05ExMaxLR", "UASImpulsSHMPowAvgMaxLR")
dVar <- "dAnnoyMean"
...
iVars <- c(absVar, "UASEventPerMin", sharpVar, tonLdVar, fluctVar, roughVar, impulsVar)
dVar <- "dAnnoyMean"
...
iVars <- c(absVar, "UASEventPerMin", sharpVar, tonalVar, fluctVar, roughVar, impulsVar)
dVar <- "dAnnoyMean"
...
if (saveplots){
  ggsave(filename="PtBdHiAnnoyDiffVarsConPermimpPtv.svg", width=8, height=6, path=file.path(outFigPath, "svg"))
  unlink("PtBdHiAnnoyDiffVarsConPermimpPtv.svg")
  
  ggsave(filename="PtBdHiAnnoyDiffVarsConPermimpPtv.pdf", width=8, height=6, path=file.path(outFigPath, "pdf"))
...
# plot res as horizontal bar chart, with Imp as y axis, Variable as x axis, Outcome as fill, and Variable_sum as order, relabel x axis with Variable names
pBar <- ggplot(resDiffB) + geom_col(aes(fill=Outcome, y=Imp, x=Order), colour='grey35',  width=0.75, show.legend=TRUE) + labs(x="Variable", y="Normalised variable permutation importance") + theme(text = element_text(family = "serif"), panel.grid=element_line(color = rgb(235, 235, 235, 100, maxColorValue = 255), linewidth = 0.25, linetype = 2), legend.position = "right") + coord_flip(ylim=c(-0.1, 2)) + scale_fill_manual(values=mycolours) + scale_x_discrete(labels=unique(rev(resDiffB$Variable)))
pBar
