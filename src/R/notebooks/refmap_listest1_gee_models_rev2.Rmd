...
predictNew_marginal <- function(model, response, newData, original_data,
  ...
  if (length(missing_vars) > 0) {
    ...
    # create weights
    w <- as.data.frame(table(marginal_raw))
    names(w)[ncol(w)] <- "w"
    
    # convert back to original types
    for (v in missing_vars) {
      if (is.numeric(marginal_raw[[v]])) {
        w[[v]] <- as.numeric(as.character(w[[v]]))
      } else if (is.factor(marginal_raw[[v]])) {
        w[[v]] <- factor(w[[v]], levels = levels(marginal_raw[[v]]))
      }
    }
    
    marginal_df <- w[w$w > 0, , drop = FALSE]
    
    marginal_df <- marginal_df[marginal_df$w > 0, , drop = FALSE]
  }
  ...
  compute_mean <- function(model_obj, marginal_df_use) {
    ...
    if (length(missing_vars) == 0) {
      if (inherits(model_obj, "glmgee") && !is.null(varest)) {
        return(predict(model_obj, newdata = stim_min, type = "response", varest = varest))
      } else {
        return(predict(model_obj, newdata = stim_min, type = "response"))
      }
    }
    ...
    for (i in seq_len(nrow(marginal_df_use))) {
      ...
      nd_i <- as.data.frame(matrix(nrow = nrow(stim_min), ncol = length(pred_all)))
      names(nd_i) <- pred_all
      
      # fill from stim data
      common <- intersect(pred_use, pred_all)
      nd_i[, common] <- stim_min[, common, drop = FALSE]
  ...
  if (method == "draws") {
    ...
    # store draw-level means
    draw_fits <- matrix(NA, nrow = nrow(stim_min), ncol = ndraws)
    
    for (i in seq_len(nrow(marginal_df))) {
      ...
      draw_fits <- draw_fits + w * mat
    }
    
    draw_fits <- draw_fits / sum(marginal_df$w)
  ...
  if (method == "bootstrap") {
    ...
    fits_boot <- matrix(NA, nrow = nrow(stim_min), ncol = nboot)
    ...
    for (b in seq_len(nboot)) {
      ...
      if (inherits(model_b, "try-error")) next
      ...
      fits_boot[, b] <- compute_mean(model_b, marginal_df_b)
    }
...
gnmgee_sigmoid <- function(formula, data, id,
  ...
  # --- 1. Build model matrix (like glm would)
  mf <- model.frame(formula, data)
  X <- model.matrix(attr(mf, "terms"), mf)
  ...
  # Remove intercept column (we'll add manually)
  term_names <- term_names[term_names != "(Intercept)"]
  
  # --- 2. Create parameter names
  p <- length(term_names) + 1  # +1 for intercept
  beta_names <- paste0("b", 0:(p-1))
  
  # --- 3. Build linear predictor expression
  rhs_terms <- paste0(beta_names[-1], "*", term_names)
  ...
  # --- 4. Build nonlinear sigmoid expression
  full_expr <- paste0(
    scale, " / (1 + exp(-(", eta_expr, ")))"
  )
  
  # --- 5. Construct new formula
  new_formula <- as.formula(
    paste(yname, "~", full_expr)
  )
  ...
  if (is.null(start)) {
    ...
    if (length(start) != p) {
      stop("Length of starting values must match number of parameters")
    }
    names(start) <- beta_names
  }
...
modelTest <- gnmgee_sigmoid(
  ...
  family = gaussian(link = 'identity'),
  corstr = "exchangeable",
  start = c(-0.7, -0.5, -0.1, -0.2, 0.3, 0.4, 0.1, -1, 0.1, 0.5)
)
...
predictMeans <- predictNew_marginal(model=model1, newData=stimDataNoAmb,
                                    response=stimDataNoAmb$dAnnoyMean,
                                    original_data=subjDataNoAmb, nboot=10,
                                    varest='bias-corrected',
                                    cluster_var='ID', seed=808)
...
gee_residplot(mABLAExTypeOpxLAExEnv1_QxLAESq.GEE,
              residual_vals=mABLAExTypeOpxLAExEnv1_QxLAESq.GEE.resP,
...
geeCluster_stemplot(mABLAExTypeOpxLAExEnv1_QxLAESq.GEE,
                    clusterVar=mABLAExTypeOpxLAExEnv1_QxLAESq.GEE.resM,
...
geeCluster_stemplot(mABLAExTypeOpxLAExEnv1_QxLAESq.GEE,
                    clusterVar=mABLAExTypeOpxLAExEnv1_QxLAESq.GEE.Cook,
...
hist(mABLAExTypeOpxLAExEnv1_QxLAESq.GEE.resP, breaks=20, main="Standardised Pearson residuals")
...
model1 <- mABdSdFdTnxdLAEL501_QSq.GEE <- glmtoolbox::glmgee(dAnnoyance ~ I(UASEvents^-1) +
                                                                LAELAF50diffScl*dTonLdECMAPowAvgBin +
                                                            dFluctECMA10ExBin +
