...
def shmLoudnessECMA(p, sampleRateIn, axisN=0, soundField='freeFrontal',
    ...
    loudnessPowAvg : 1D or 2D array
        Time-averaged overall loudness arranged as [loudness(, channels)].
    ...
    for chan in range(chansIn):
        ...
        maxLoudnessFuncel = a/(np.nanmax(specTonalLoudness[:, :, chan]
...
def shmLoudnessECMAFromComp(specTonalLoudness, specNoiseLoudness,
    ...
    specTonalLoudness : 2D or 3D array
        ...
        a sound pressure signal (single mono or single stereo audio)
