...
% Input is a calibrated single mono or single stereo audio (sound pressure)
% time-series, p.
% Calculates tonality according to ECMA-418-2:2022 (using the Hearing Model
% according to Sottek).
%
% Inputs
% ------
...
NB = [0, 1, 2*ones(1,14), ones(1,9), zeros(1,28);...
      2*ones(1,16), ones(1,9), zeros(1,28)];
...
for chan = size(pn_om, 2):-1:1
    ...
    pn_romz = [pn_romz(:, 1:5), pn_romz(:, 2:18), pn_romz(:, 16:26), pn_romz(:, 26:53)];
    ...
    NBi_temp = [1, 1, 1, 6:18, 23:31, 34:61; 3:5, 10:22, 25:33, 34:61];  % these are the (duplicated) indices corresponding with the NB bands around each z band
    ...
    for zz = 61:-1:1
        ...
        pn_romzt = pn_romz(i_start:end, zz);
        ...
        plz = sqrt((2/sz_b_temp(zz))*sum(pn_lz.^2, 1));
        % Section 5.1.8 Equations 23 & 24 ECMA-418-2:2022
        Nz = c_N*(plz/20e-6).*prod((1+(plz./p_t).^a).^((diff(v)/a)'));
        % Section 5.1.9 Equation 25 ECMA-418-2:2022
        Nz_basis = Nz - LTQz_temp(zz);
        Nz_basis((Nz_basis-LTQz_temp(zz)) < 0) = 0;

        % Section 6.2.2 Equation 30 ECMA-418-2:202
        phim_lz_temp{zz} = Nz_basis.*phim_lz;
    ...
    for zz = 53:-1:1 % Loop through 53 critical band filtered signals
        ...
        mask = 2*max(Phik_lztau(1:sz_b(zz),:), [], 1)/(M/2) <= phim_lz_avg(1, :);
        Nlz_tonal(mask) = 2*max(Phik_lztau(1:sz_b(zz), mask), [], 1)/(M/2);  % first estimation of specific loudness of tonal component in critical band
    
        % Section 6.2.5 Equations 38 & 39 ECMA-418-2:2022
        [~, kz_max] = max(Phik_lztau(1:sz_b(zz),:), [], 1);
        ...
        if i_interp(zz) > 1
            ...
            fz_ton = resample(fz_ton, i_interp(zz), 1);
        end
        ...
        % implemented terms below incorporate an amendment to the standard
        % to compensate for the start zero-padding, which results in
        % improved time alignment of the processed signals with the input.
        % For strict standard (as-written) adherence, the corresponding
        % commented lines below should be uncommented.
        l_end = ceil((n_samples + n_zeross)/r_sre*r_sd) + 1;  % Equation 40 ECMA-418-2:2022 (edited to account for start zero-padding)
%         l_end = ceil(n_samples/r_sre*r_sd) + 1;  % Equation 40 ECMA-418-2:2022 (uncomment line for strict standard adherence)
        l_start = floor(n_zeross/r_sre*r_sd) + 1;  % Additional term to remove start zero-padding lag
%         l_start = 1;  % (uncomment line for strict standard adherence)
    ...
    Ttz(:, 1:57, chan) = 0;  % Discard (zero) first ~300 ms (Section 6.2.9 point 1)
    ftz_ton(:, 1:57, chan) = 0;
    for zz = 53:-1:1
