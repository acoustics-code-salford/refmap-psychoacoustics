function sone = shmPhon2Sone(phon)
% sone = shmPhon2Sone(phon)
%
% Returns the sensory loudness value(s) in sone corresponding with loudness
% level(s) in phon, according to the Sottek Hearing Model as defined in
% ECMA-418-2:2025.
%
% Inputs
% ------
% phon : float or array of floats
%   Loudness level(s) determined using Sottek's Hearing Model.
%
% Returns
% -------
% 
% sone: float or array of floats
%   Loudness value(s), corresponding with input phon.
%
% Assumptions
% -----------
% The input phon level(s) represents loudness levels according to the
% Sottek Hearing Model. The output sone is returned based on linear
% interpolation between phon-to-sone values calculated at discrete
...
if ismatrix(phon)  % flatten matrix to vector
    soneReshape = true;
    phonSize = size(phon);
    phon = phon(:);
end

sone = interp1(phonRange, soneRange, phon, 'linear', 'extrap');
